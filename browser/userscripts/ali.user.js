...
const clearLink = (href, com = true, shit = true) => {
  ...
  if (shit && newLink.join('/').split('?').length > 1) {
    const newShit = newLink
      ...
      .filter((param) => !blacklistedUrlParams.includes(param[0]));
...
const old = (link) => {
  ...
  if (
    url.indexOf('ru.aliexpress.com') > -1 ||
    url.indexOf('pt.aliexpress.com') > -1 ||
    url.indexOf('es.aliexpress.com') > -1 ||
    url.indexOf('id.aliexpress.com') > -1 ||
    url.indexOf('fr.aliexpress.com') > -1
  ) {
    console.log(url);
    if (
      (url.indexOf('.aliexpress.com/item/') > -1 || url.indexOf('.aliexpress.com/store/product/') > -1) &&
      url.indexOf('isOrigTitle') == -1 &&
      url.indexOf('isOrig') == -1
    ) {
      if (url.indexOf('?') > -1) {
